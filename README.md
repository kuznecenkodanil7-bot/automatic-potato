...
## Добавлено в версии gui-timer-ipban

- При нажатии **«Вызвать на проверку»** отправляются команды проверки и запускается OBS-запись.
- Если OBS подтвердил старт записи, над хотбаром появляется таймер: `Идёт запись: 00:00`.
- Таймер обновляется каждую секунду и исчезает после остановки записи.
- После выдачи **IPBan** OBS-запись останавливается автоматически.
- Исключения для автоостановки OBS: причины `бот` и `3.8`.
- GUI затемнён полупрозрачным фоном, есть правая панель статистики, список недавних игроков и подсветка выбранного типа наказания.
