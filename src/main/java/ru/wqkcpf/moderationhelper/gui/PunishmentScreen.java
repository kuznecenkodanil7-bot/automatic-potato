...
public final class PunishmentScreen extends BaseModScreen {
    ...
    private void callCheck() {
        ...
        boolean recordingStarted = ModerationHelperClient.OBS.startRecording();
        ModerationHelperClient.RECENT_PLAYERS.add(nick);
        if (recordingStarted) {
            ModerationHelperClient.chat("Игрок вызван на проверку, OBS-запись и таймер запущены: " + nick);
        } else {
            ModerationHelperClient.chat("Игрок вызван на проверку, но OBS-запись не стартовала: " + nick);
        }
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        renderCategoryHint(context, x + 24, y + 255);
