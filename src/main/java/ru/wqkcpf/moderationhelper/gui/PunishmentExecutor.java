...
public final class PunishmentExecutor {
    ...
    public static void execute(PunishmentDraft draft) {
        ...
        if (draft.type == PunishmentType.IPBAN) {
            if (shouldStopObsAfterIpBan(reason)) {
                // После IPBan запись автоматически останавливается.
                // Исключения: причины "бот" и "3.8".
                ModerationHelperClient.OBS.stopRecording();
                ModerationHelperClient.chat("OBS-запись остановлена после IPBan.");
            } else {
                ModerationHelperClient.chat("OBS-запись НЕ остановлена: исключение для причины " + reason + ".");
            }
        }
